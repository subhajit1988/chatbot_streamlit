...
def construct_prompt(question: str, context_embeddings: dict, df: pd.DataFrame, head) -> str:
    """
    Fetch relevant 
    """
    ...
    i = 1
    #print("DOCUMENT_SELECTION:::::::", most_relevant_document_sections)
    #print(df.head())
    for _, section_index in most_relevant_document_sections:
       # print("SECTION_INDEX:::::::", section_index)
        # Add contexts until we run out of space.        
        document_section = df.iloc[int(section_index)]
        ...
        if isinstance(document_section['summarized'], (pd.DataFrame, pd.Series)):
            document_sect = document_section['summarized'].values[0]
        else:
            document_sect = document_section['summarized']

        chosen_sections_len += toknum + separator_len
        if chosen_sections_len > MAX_SECTION_LEN:
            break
        context_heading = " Context {}-- Title: {}, Context Body:".format(i,section_index[0])
        i += 1
    ...
    print("CHOSEN_SECTIONS::::",str_chosen_sections) 
...
#PROMPTS
code_header = f"""You are an AI assistant with up-to-date knowledge for Host Home Affiliate Information, a lawfully and ethically conducting business in accordance with Eli Lilly. You assist the LMC and CMS programe 
                department employees of Eli Lilly. Generate your answer ONLY with the facts based solely on the provided embeddings in elaborated way. Your responses are rooted in 
                integrity, excellence and respect for individuals, ensuring accuracy and reliability in all information shared.
                Answer the user's query using only this context.\
                *** CONTEXT ***\
   
                """  
code_answering_instructions = """
[IMPORTANT INSTRUCTIONS]:
- If a query does not pertain to legitimate Host Home Affiliate Information related inquiry, respond with: "I'm sorry, I can only assist with Host Home Affiliate Information related queries."
- If the necessary information for answering "Q:" is not in the provided "CONTEXT", say: "Sorry, my knowledge base lacks the relevant information to address your question."
- Upon finding the required information in the "CONTEXT", address the query or complete the task ensuring you also follow the next point.
...
- You excel in providing expert advice on this drug based on user inquiries.
...
def preprocess_user_input(user_input, message_history, temp=0.3, max_tok=300):
    ...
    system_message = """Enhance the user's query based on the conversation history. The conversation history consists of a user asking questions related to Host Home Affiliate Information, a drug containing Baricitinib as its active ingredient.
                    ...
                    Focus on refining and providing full context in the question. Only return the optimized query. Do not enhance question with previous reference information or enchance any with any information outside of chat history. 
                    Only refine and optimize when there is information in history to do so.
                    """
...
def ask_gpt(user_input, embeddings, df, header, instructions, message_hist, max_selection=5000, max_tokens_output=800, temperature=0.3):
    ...
    random_sleep()
    #print("ASK GPT:::::: construct_prompt::::::::::::::::\n", df["summarized"].head(2))
    sys_prompt = construct_prompt(user_input, embeddings, df, header)
...
def input_checker2(message_hist, user_input, temp=0):
    ...
    if len(message_hist) < 2:
        ...
        Your task is to evaluate the dialogue history between an employee and a Medical Affairs Europe employee assistant."""
...
def basic_chain_parallel_single(message_hist, user_input,code_embeddings, df_code):
    ...
    if checker == "a":
        ...
        last_response = "I'm sorry, I can only assist with HR GBS related queries."
        #print("\n***INTENT_CHECKER=", checker, "\n")
        return last_response, message_hist   
